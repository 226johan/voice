...
int text_to_speech(const char *src_text,const char *des_path,const char *params){
    ...
    if(MSP_SUCCESS != ret){
        printf("QTTSSessionBegin failed, ret=%d\n", ret);
        fclose(fp);
        return ret;
    }
    ret = QTTSTextPut(sessionID, src_text, (unsigned int)strlen(src_text), NULL);
    if(MSP_SUCCESS != ret){
        printf("QTTSTextPut failed, ret=%d\n", ret);
    ...
    printf("\n合成结束\n");
    if(MSP_SUCCESS != ret){
        printf("QTTSAudioGet failed, ret=%d\n", ret);
    ...
    if(MSP_SUCCESS != ret){
        printf("QTTSSessionEnd failed, ret=%d\n", ret);
    }
    return ret;
}
